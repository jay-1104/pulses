...
function useBean(
  ...
  const y = useTransform(progress, [0.26 + delay, 0.64 + delay], [-200, landingDepth]);
  const x = useTransform(progress, [0.24 + delay, 0.5 + delay], [offset * 0.18, offset]);
  const opacity = useTransform(
    progress,
    [0.24 + delay, 0.3 + delay, 0.68 + delay, 0.74 + delay],
    [0, 1, 1, 0]
  );
  const rotate = useTransform(progress, [0.26 + delay, 0.64 + delay], [baseRotate, baseRotate * 1.65]);
  const squish = useTransform(progress, [0.56 + delay, 0.64 + delay, 0.7 + delay], [1, 0.72, 1]);
...
export function Scene() {
  ...
  const canOpacity = useTransform(smoothProgress, [0, 0.1], [0, 1]);
  const canScale = useTransform(smoothProgress, [0, 0.18], [0.84, 1]);
  const canLift = useTransform(smoothProgress, [0, 0.26], [60, -6]);
  const canTilt = useTransform(smoothProgress, [0.26, 0.48], [0, -52]);

  // Lid gently lifts and tilts open.
  const lidLift = useTransform(smoothProgress, [0.14, 0.32], [0, -140]);
  const lidRotate = useTransform(smoothProgress, [0.14, 0.38], [0, -34]);
  const lidOpacity = useTransform(smoothProgress, [0.1, 0.18], [0, 1]);

  // Frying pan fades in below the can.
  const panOpacity = useTransform(smoothProgress, [0.3, 0.44], [0, 1]);
  const panRise = useTransform(smoothProgress, [0.3, 0.5], [140, 0]);
  const flameGlow = useTransform(smoothProgress, [0.44, 0.58], [0, 1]);
  const panTilt = useTransform(canTilt, [-52, 0], [-6, 0]);
  const sizzleOpacity = useTransform(smoothProgress, [0.48, 0.62], [0, 1]);
  const sizzleScale = useTransform(smoothProgress, [0.5, 0.66], [0.8, 1.28]);

  // Steam lifts in soft waves once beans land.
  const steamOpacity = useTransform(smoothProgress, [0.52, 0.68], [0, 1]);
  const steamDrift = useTransform(smoothProgress, [0.52, 0.86], [40, -30]);

  // Final message fades and slides into view.
  const messageOpacity = useTransform(smoothProgress, [0.62, 0.82], [0, 1]);
  const messageRise = useTransform(smoothProgress, [0.62, 0.82], [40, 0]);

  const beanMotions: BeanMotion[] = [
    useBean(smoothProgress, 0, -52, -18, 254),
    useBean(smoothProgress, 0.03, -10, 14, 248),
    useBean(smoothProgress, 0.06, 38, -6, 252),
    useBean(smoothProgress, 0.09, -22, 18, 246),
    useBean(smoothProgress, 0.12, 30, -12, 250),
  ];

  return (
    <div ref={containerRef} className="relative mt-12 h-[180vh]">
